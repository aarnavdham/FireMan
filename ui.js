  ...
  class UIManager {
    ...
    handleHostRoom() {
      ...
      net.hostRoom(code).then(() => {
        ...
        // Start with just the host (playerCount=1). Joiners auto-expand the count
        // when they connect, so we don't pre-create empty "waiting" slots.
        multiplayer.setPlayerCount(1);
        if (statusEl) {
    ...
    _wireNetCallbacks() {
      ...
      // Host: a new client connected (called once, from the 'hello' message)
      net.onPeerJoin = (peerId, conn, name) => {
        // Guard against double-add: if this peerId is already in a slot, skip
        const existing = multiplayer.slots.findIndex(s => s.peerId === peerId);
        if (existing >= 0) {
          // Already added — just update the name if provided
          if (name) multiplayer.slots[existing].name = name;
          net.broadcastLobby(multiplayer);
          this.refreshLobby();
          return;
        }
        // Find the first available slot (not local, no peer assigned) starting from slot 1
        let slotIdx = -1;
        for (let i = 1; i < multiplayer.playerCount; i++) {
          if (!multiplayer.slots[i].isLocal && !multiplayer.slots[i].peerId) {
            slotIdx = i;
            break;
          }
        }
        // If no empty slot within current count, expand (up to 5)
        if (slotIdx === -1) {
          if (multiplayer.playerCount >= 5) {
            net.sendTo(conn, { t: 'chat', a: 'System', m: 'Room is full.' });
            return;
          }
          slotIdx = multiplayer.playerCount;
          multiplayer.setPlayerCount(multiplayer.playerCount + 1);
        }
      ...
      net.onPeerLeave = (peerId) => {
        ...
        if (idx >= 0) {
          const leftName = multiplayer.slots[idx].name;
          multiplayer.slots[idx].peerId = null;
          multiplayer.slots[idx].isLocal = false; // stays empty, not reclaimed
          // If this was the last slot, shrink the player count
          while (multiplayer.playerCount > 1 &&
                 !multiplayer.slots[multiplayer.playerCount - 1].peerId &&
                 !multiplayer.slots[multiplayer.playerCount - 1].isLocal) {
            multiplayer.setPlayerCount(multiplayer.playerCount - 1);
          }
          net.broadcastLobby(multiplayer);
          this.refreshLobby();
          multiplayer.sendChat('System', `${leftName} left`);
          this.refreshChat();
        }
      };
